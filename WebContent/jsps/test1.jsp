...
function onSearch(obj){//js函数开始
  setTimeout(function(){//因为是即时查询，需要用setTimeout进行延迟，让值写入到input内，再读取
	...
	<%
		...
		System.out.println("childs.length（371）:" + childs.length);
	...
	<%
		...
		if (parameter[1] == "000") {//一级目录
			...
			<script>mytable="mytable1"; 
					...
					<%if (chosenIf == 1) {
						System.out.println("chosenIf（为1-已被其它医生选择）：" + chosenIf);
						chosenName = new String[2];
						chosenIf = 0;
					}%>
			</script>
			<table id="mytable1" style="width:100%;cellspacing:0">
			...
			<%
				...
				//第一级目录的判断
				if (parameter[1] == "000") {
					if ((childs[i].getName().contains("_" + thisDoctor))
							&& (childs[i].getName().split("_")[1].equals(thisDoctor))) {
						//包含_的
			...
			<%
						...
						System.out.println(
								childs[i].getAbsolutePath().replaceAll("\\\\", "/") + "<-绝对路径");
			...
			%>
						<tr align="center">
							...
							<td><%=marknum_%></td>
							<%
								String webUrl = websit + "param0=" + URLEncoder.encode(parameter[0], "UTF-8") + "&"
														+ "param1=" + parameter[1] + "&" + "param2=" + parameter[2] + "&" + "param3="
														+ parameter[3] + "&" + "param4=" + parameter[4] + "&" + "param5=" + parameter[5];
							...
							<td><a style="color: #66ff22; text-decoration: none;"
								href=<%=webUrl%>><button class="pointer"
			...
			%>
					<tr align="center">
						...
						<%
							System.out.println(
													URLEncoder.encode(parameter[0], "UTF-8") + "22222222222222222222222222222");
						...
						<td><a
							href=<%=websit + "param0=" + URLEncoder.encode(parameter[0], "UTF-8") + "&" + "param1="
									...
									class="pointer"
									style="background-color: black; color: white; width: 60px; height: 28px">
									<font>选择</font>
								</button></a></td>
						<%
							}
		...
		<div style="margin-top:2%">
			<input type="button" class="pointer"
				style="background-color: green; color: white; width: 100px; height: 28px"
				name="Submit"
				onclick="javascript:window.location.href='<%=returnUrl2%>'"
				value="返回上一页">
		</div>
		<%
			}
